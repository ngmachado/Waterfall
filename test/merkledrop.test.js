...
contract("MerkleDrop - Claims testings", accounts => {
    ...
    it("Case #0.1 - should rever if register two identical userSet", async() => {
        const mintAmount = toWei("1000");
    ...
    it("Case #0.2 - should rever if register without correct information", async() => {
        const mintAmount = toWei("1000");
        const token = await tokensFactory.new(mintAmount, {from: tokenProvider});
        const drop = await merkleFactory.new();
        const userSet = [
    ...
    it("Case #3 - reverts if reuse claim", async () => {
        const mintAmount = toWei("1000");
        const token = await tokensFactory.new(mintAmount, {from: tokenProvider});
        const drop = await merkleFactory.new({from:tokenProvider});
        await token.approve(drop.address, mintAmount, {from: tokenProvider});
    ...
    it("Case #4 - revert if fake claim", async () => {
        const mintAmount = toWei("1000");
        const token = await tokensFactory.new(mintAmount, {from: tokenProvider});
        const drop = await merkleFactory.new({from:tokenProvider});
        await token.approve(drop.address, mintAmount, {from: tokenProvider});
    ...
    it("Case #3 - revert if provider don't have balance to transfer", async () => {
        const mintAmount = 1;
        const token = await tokensFactory.new(mintAmount, {from: tokenProvider});
        const drop = await merkleFactory.new({from:tokenProvider});
    ...
    it("Case #x.x - claim out if time", async () => {
        const mintAmount = toWei("1000");
        const token = await tokensFactory.new(mintAmount, {from: tokenProvider});
        const drop = await merkleFactory.new({from:tokenProvider});
        await token.approve(drop.address, mintAmount, {from: tokenProvider});
